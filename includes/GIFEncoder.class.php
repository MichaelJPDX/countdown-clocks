...
/*
**  Array of possible time zones for setting TZ in calling clocks
*/
$tzMap = Array("ET" => "America/New_York",
			   "CT" => "America/Chicago",
			   "MT" => "America/Denver",
...
/*
** Calculate time difference between two timestamps and output it in 'clock' format
*/
function myDateDiff($date1, $date2, $colons = true) {
	if ($colons) $sep = ":";
	else $sep = " ";
	if($date1 > $date2) {
		$tmp = $date1;
	...
	$diffh = date('G', $date2) - ((int) date('G', $date1));
	if($diffh < 0) {
	  	$diffd -= 1;
	  	$diffh = $diffh + 24;
	}
	$diffi = ((int) date('i', $date2) - ((int) date('i', $date1)));
	if($diffi < 0) {
	  	$diffh -= 1;
	  	$diffi = $diffi + 60;
	}
	$diffs = ((int) date('s', $date2) - ((int) date('s', $date1)));
	if($diffs < 0) {
	  $diffi -= 1;
	  $diffs = $diffs + 60;
	}
	$diff = $diffd . $sep . str_pad($diffh, 2, '0', STR_PAD_LEFT) . $sep . 
			str_pad($diffi, 2, '0', STR_PAD_LEFT) . $sep . str_pad($diffs, 2, '0', STR_PAD_LEFT);
	return $diff;
}
/*
** Calculate time difference between two timestamps and output it in as an ARRAY of d,h,m.s
*/
function myDDarray($date1, $date2) {
	if (phpversion() < 5.3) {
		if($date1 > $date2) {
			$tmp = $date1;
			$date1 = $date2;
		...
		$diffArray[1] = str_pad($diffh, 2, '0', STR_PAD_LEFT);
		$diffArray[2] = str_pad($diffi, 2, '0', STR_PAD_LEFT);
		$diffArray[3] = str_pad($diffs, 2, '0', STR_PAD_LEFT);
	} else {
		$start_date = new DateTime(date('Y-m-d H:i:s', $date2));
		$since_start = $start_date->diff(new DateTime(date('Y-m-d H:i:s', $date1)));
		$diffArray[0] =  $since_start->days;
		$diffArray[1] =  str_pad($since_start->h, 2, '0', STR_PAD_LEFT);
		$diffArray[2] =  str_pad($since_start->i, 2, '0', STR_PAD_LEFT);
		$diffArray[3] =  str_pad($since_start->s, 2, '0', STR_PAD_LEFT);
	}
	return $diffArray;
}
/*
**
*/
function myDateDiffWords($date1, $date2) {
	if($date1 > $date2) {
		$tmp = $date1;
		$date1 = $date2;
		$date2 = $tmp;
	...
	$diff = $diffd . 'days, ' . str_pad($diffh, 2, '0', STR_PAD_LEFT) . ' hours, ' . 
			str_pad($diffi, 2, '0', STR_PAD_LEFT) . ' minutes ' . str_pad($diffs, 2, '0', STR_PAD_LEFT) 
			. ' seconds';
	return $diff;
}
